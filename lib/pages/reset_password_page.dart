...
class _ResetPasswordPageState extends State<ResetPasswordPage> {
  ...
  Future<void> _sendCode() async {
    ...
    try {
      await _api.sendResetPasswordCode(mail: mail);
      _startCountdown();
      if (!mounted) {
        return;
      }
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('重置验证码已发送，请查收邮箱')));
  ...
  Future<void> _resetPassword() async {
    ...
    try {
      ...
      setState(() {
        _errorText = error.message ?? '重置密码失败';
      });
    } catch (_) {
      if (!mounted) {
        return;
