...
class _ForgotUsernamePageState extends State<ForgotUsernamePage> {
  ...
  Future<void> _submit() async {
    ...
    try {
      await _api.forgetUsername(mail: mail);
      if (!mounted) {
        return;
      }
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('用户名已发送到邮箱，请注意查收')));
      ...
      setState(() {
        _errorText = error.message ?? '找回用户名失败';
      });
    } catch (_) {
      if (!mounted) {
        return;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('找回用户名')),
      body: SafeArea(
        child: Center(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      TextButton(
                        ...
                        child: const Text('记得用户名但忘记密码？去重置'),
                      ),
                      Text(
